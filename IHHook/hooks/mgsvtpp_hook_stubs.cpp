...
namespace IHHook {
	...
	int lua_iscfunctionHook(lua_State * L, int idx) {
		...
		return lua::lua_iscfunction(L, idx);
	}//lua_iscfunctionHook
	...
	int lua_equalHook(lua_State * L, int index1, int index2) {
	
		return lua::lua_equal(L, index1, index2);
	}//lua_equalHook
	...
	lua_CFunction lua_tocfunctionHook(lua_State * L, int idx) {
		...
		return lua::lua_tocfunction(L, idx);
	}//lua_tocfunctionHook
	...
	int luaL_errorHook(lua_State * L, const char * fmt, ...) {
		...
		return lua::luaL_error(L, fmt);
	}//luaL_errorHook
	
	int luaL_checkoptionHook(lua_State * L, int narg, const char * def, char * * lst) {
	
		return lua::luaL_checkoption(L, narg, def, lst);
