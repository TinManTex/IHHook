...
namespace IHHook {
	...
	std::map<std::string, void*> funcPtrPtrs{};//tex pointers to the funcptrs we use to call the original, need to map it so we can still call it when MH_CreateHook pushes it out of the way with a hook . set in SetFuncPtrs (generated by ExportInfo)
